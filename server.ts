...
export const loadDatabase = (): Database => {
  try {
    if (fs.existsSync(dbPath)) {
      ...
      const defaultDb: Database = { wheelUnlocked: false, prizes: [], spinRequests: [] };
...
app.post('/api/prizes', (req: Request, res: Response) => {
  const db = loadDatabase();
  db.prizes = req.body;
  saveDatabase(db);
...
app.post('/api/spin-requests', (req: Request, res: Response) => {
  const db = loadDatabase();
...
app.post('/api/unlock-wheel', (_req: Request, res: Response) => {
  const db = loadDatabase();
...
app.get('/api/wheel-status', (_req: Request, res: Response) => {
  const db = loadDatabase();
  res.json({
...
app.post('/api/lock-wheel', (_req: Request, res: Response) => {
  const db = loadDatabase();
